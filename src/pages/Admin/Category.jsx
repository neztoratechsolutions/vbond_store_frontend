...
function Category() {
  ...
  const openEditModal = (category) => {
    ...
    setModalInputs([{ tempId: Date.now(), name: category.name }]);
    setIsModalOpen(true);
  };
  ...
  const removeCategoryField = (tempId) => {
  ...
  const handleSaveCategories = async (e) => {
    ...
    try {
      if (editingCategoryId) {
        // UPDATE (PUT request)
        const payload = { name: validCategories[0].name, is_active: true };
        ...
        if (res.ok) {
          ...
          setCategories(categories.map(cat => 
            cat.id === editingCategoryId ? updatedCat : cat
          ));
          closeModal();
          Swal.fire({
            icon: 'success',
          ...
          Swal.fire({ icon: 'error', title: 'Failed', text: 'Failed to update category.', confirmButtonColor: '#DD2476' });
        }
      } else {
        // CREATE BULK (POST request)
        const payload = {
          categories: validCategories.map(field => ({ name: field.name, is_active: true }))
        };
        ...
        if (res.ok) {
          ...
          Swal.fire({
            icon: 'success',
            title: 'Created!',
            text: 'Categories have been created successfully.',
            confirmButtonColor: '#DD2476',
            timer: 1500,
            showConfirmButton: false
          });
        } else {
          Swal.fire({ icon: 'error', title: 'Failed', text: 'Failed to create categories.', confirmButtonColor: '#DD2476' });
        }
      }
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    Swal.fire({
      ...
      confirmButtonColor: '#DD2476', // Red/Pink theme color
      cancelButtonColor: '#6c757d',
      confirmButtonText: 'Yes, delete it!'
    }).then(async (result) => {
      if (result.isConfirmed) {
        // Show loading spinner if request takes time
        Swal.fire({
          title: 'Deleting...',
        ...
        try {
          ...
          if (res.ok) {
            ...
            Swal.fire({ icon: 'error', title: 'Failed', text: 'Failed to delete category.', confirmButtonColor: '#DD2476' });
          }
        } catch (err) {
              ...
              <table className="table table-hover align-middle">
                ...
                <tbody>
                  {categories.length > 0 ? (
                      ...
                      <tr key={cat.id} style={{ borderBottom: "1px solid #f8f9fa" }}>
                        ...
                        <td>
                          <span 
                            ...
                            style={{ 
                              background: cat.is_active ? "rgba(40, 167, 69, 0.1)" : "rgba(108, 117, 125, 0.1)", 
                              color: cat.is_active ? "#28a745" : "#6c757d",
                              fontWeight: "500"
                            }}
                          >
                        ...
                        <td className="text-end">
                          <div className="d-flex justify-content-end">
                            {/* Edit Button */}
                            <button className="btn btn-sm btn-link text-primary p-1 me-2" onClick={() => openEditModal(cat)}>
                            ...
                            {/* Delete Button */}
                            <button className="btn btn-sm btn-link text-danger p-1" onClick={() => handleDelete(cat.id)}>
                    ...
                    <tr>
                      <td colSpan="4" className="text-center text-muted py-5">No categories found. Click "Add Category" to create one.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="modal-content-custom" style={{
            ...
            <div className="d-flex justify-content-between align-items-center p-4 border-bottom">
              <h5 className="fw-bold text-dark mb-0">
                {editingCategoryId ? "Edit Category" : "Add Multiple Categories"}
              </h5>
            ...
            <form onSubmit={handleSaveCategories} className="p-4">
              ...
              <div className="dynamic-fields-container" style={{ maxHeight: "300px", overflowY: "auto" }}>
                {modalInputs.map((field, index) => (
                  <div key={field.tempId} className="d-flex align-items-center mb-3">
                    ...
                    {!editingCategoryId && modalInputs.length > 1 && (
                      <button 
                        type="button" 
                        className="btn btn-link text-danger ms-2 p-2"
                        onClick={() => removeCategoryField(field.tempId)}
                      >
              ...
              <div className="d-flex justify-content-end mt-4 pt-3 border-top">
                ...
                >
                  {editingCategoryId ? "Update Category" : "Save All Categories"}
                </button>
              </div>
            </form>
          </div>
